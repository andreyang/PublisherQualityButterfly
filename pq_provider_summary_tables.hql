...
drop table if exists publisher_provider_quality_sum_temp_butterfly;
create table publisher_provider_quality_sum_temp_butterfly as
select 
	coalesce(p.domain_id,0) domain_id,
	ct.company_id,
	a.event_date,
...
from ay_temp_butterfly_pq a
        ...
        on (a.video_id = ct.uuid) 
        left outer join client_portal.players p
        on (a.player_id = p.uuid)
